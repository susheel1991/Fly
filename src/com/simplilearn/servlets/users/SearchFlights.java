...
public class SearchFlights extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String query = "select f from Flightinfo f" + 
					" inner join Airline a" + 
					" on f.airline=air_id" + 
					" and f.Number_of_seats > 0";
					...
					if (price !=null && price.trim().length() > 0) {
						try {
							...
							String priceQuery = " and f.fare <= " + priceParsed;
					...
					if (date !=null && date.trim().length()> 0) {
						try {
							...
							String dateQuery = " and f.Date_of_Departure >= '" + dateSql + "'";
